...
class AlignmentViewer:
    ...
    def _slice_payload(self, query: Mapping[str, list[str]]) -> dict[str, Any]:
        ...
        suggested_depth = suggested_alignment_depth(self.metadata, state, plane)
...
async function requestPreview(){try{const body={plane,depth:current.depth,target_distance:calibration.distance,points:calibration.points};preview=await api("/api/calibration-preview",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(body)});for(const name of ["plan","cross"]){layers[name].matrix.copy(displayMatrix(preview.operations[name]));layers[name].matrixAutoUpdate=false}const p=preview.measurements;document.getElementById("preview-text").textContent=`Plan: ${p.plan.measured_distance.toFixed(5)} -> ${p.plan.target_distance.toFixed(5)} (x${p.plan.scale_factor.toFixed(6)}). Cross: ${p.cross.measured_distance.toFixed(5)} -> ${p.cross.target_distance.toFixed(5)} (x${p.cross.scale_factor.toFixed(6)}). Each first selected point remains fixed. No relative rotation or translation is applied.`;configureRotationSlider();document.getElementById("preview-dialog").showModal();statusBox.textContent="Calibration preview."}catch(error){cancelCalibration();statusBox.textContent=error.message}}
...
document.querySelectorAll("[data-plane]").forEach(button=>button.addEventListener("click",()=>{clearRotationPreview();plane=button.dataset.plane;loadSlice({refit:true})}));
